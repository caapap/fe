const ja_JP = {
  ...
  webhooks: {
    help_content:
      "コールバック機構、星相と他のシステムとの統合に使用されます。星相がアラートイベントを生成した後、各コールバックアドレスにプッシュします。星相がコールバックする際に使用する HTTP メソッドは POST です。HTTP リクエストボディにアラートイベントの内容を JSON フォーマットで置きます。イベントデータの構造については、[ここ](https://github.com/ccfos/nightingale/blob/main/models/alert_cur_event.go#L19)を参照してください。星相とネットワークが互通可能なマシン（例えば、その IP が 10.1.2.3 であると仮定）を探し、そこで nc を使用してポートを立ち上げることができます。例えば、`nc -k -l 4321` です。これで、`http://10.1.2.3:4321` をコールバックアドレスに設定し、アラートルールを作成し、トリガーが発生すると、星相がこのアドレスにコールバックします。すると、nc コマンドの出力に星相がコールバックしてきた詳細なデータフォーマットを見ることができます。",
    title: "コールバックアドレス",
    enable: "有効",
    note: "備考",
    url: "URL",
